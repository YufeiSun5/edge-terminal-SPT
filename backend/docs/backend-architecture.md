...
- `detection_run_features`: per-run numeric feature rows grouped by variable. The first slice stores `sample_count`, `avg_value`, `min_value`, `max_value`, `first_sample_time`, and `last_sample_time`, calculated from persisted history rows after a run stops or when explicitly refreshed.
- `detection_limit_alarms`: per-run limit alarm lifecycle records. One row represents one variable's limit violation from `first_seen_at` until `recovered_at`; it freezes task, Project, standard item, variable display name, check method, alarm type/level, start/peak/recover values, limit value, quality, status, and duration. Business limit alarm enter/recover is generated from the in-memory run context and written asynchronously through the `Alarm` queue, with enter events batched and recover events ordered after pending enters. It is intentionally named for limit alarms, not generic alarms.
...
- `sys_notification_recipients`: per-user read state for persisted notifications. Recipient generation now honors `target_type=all|user|role|project`: `all` fans out to all enabled local edge users, `user` targets one enabled local user ID, `role` targets enabled local users with that role, and `project` currently falls back to all enabled local users because project membership is not modeled yet. Each recipient row stores its own `read_at`.
...
- `GET /api/v1/task-flow-runs`: list task-flow execution records. Requires `system_settings`; supports project, flow, trigger, status, origin, time, limit, and offset filters.
- `GET /api/v1/task-flow-runs/{id}`: get one task-flow execution record.
- `GET /api/v1/task-flow-runs/{id}/sql-logs`: list SQL statements executed by one task-flow run through the shared repository connection.
- `DELETE /api/v1/variables/{variable_id}`: delete a wrongly discovered variable.
- `PATCH /api/v1/variables/{variable_id}/assignment`: assign a variable to a virtual Project and enable it.
...
- `PATCH /api/v1/projects/{id}`: update a virtual Project group, including UI-facing display names.
...
- `POST /api/v1/detection-standards/{id}/favorite`: favorite one detection standard for the current user.
- `DELETE /api/v1/detection-standards/{id}/favorite`: remove the current user's favorite link.
- `POST /api/v1/detection-standards`: create a detection standard, optionally with items. Item payloads support `alarm_enabled`, `check_cycle_ms`, `check_on_start`, `check_method`, `target_value`, `limit_deadband`, `violation_hold_ms`, `recover_hold_ms`, and `quality_policy`; defaults are `alarm_enabled=true`, `check_cycle_ms=0`, `check_on_start=true`, `numeric_range`, and `ignore_bad`.
- `GET /api/v1/detection-standards/{id}`: get one detection standard with items.
- `PATCH /api/v1/detection-standards/{id}`: update one detection standard header.
- `PUT /api/v1/detection-standards/{id}/items`: replace all items under a standard and increment its version. Invalid `check_method`, invalid `quality_policy`, negative deadband, or negative hold times return `400`.
- `DELETE /api/v1/detection-standards/{id}`: delete one unused detection standard and its config items; returns `409 Conflict` when the standard has run snapshots.
...
- `PATCH /api/v1/report-templates/{id}`: update one Excel report template metadata record.
- `DELETE /api/v1/report-templates/{id}`: delete one unused template; returns `409 Conflict` when referenced by standards, tasks, or generated reports.
...
- `PATCH /api/v1/gateway-configs/{gateway_id}`: update one MQTT/KIO source and restart/stop it according to `enabled`.
- `DELETE /api/v1/gateway-configs/{gateway_id}`: delete one MQTT/KIO source and stop it.
...
- `GET /api/v1/notifications`: list the current user's persisted edge notifications. Requires `view_realtime`; supports `unread`, `type`, `level`, `project_id`, `limit`, and `offset`, and returns `{items,total,limit,offset}`. `items[].payload` is returned as a JSON object.
...
- `GET /api/v1/detection-runs/{id}/features`: get or refresh numeric feature rows for one run. The backend derives average/min/max/sample-count values from persisted history rows grouped by variable.
...
- `GET /api/v1/detection-runs/{id}/storage-routes`: list the immutable storage route snapshot for one run. The response is `{items,count}` and each item shows the actual target table, dynamic column, trigger mode, cycle, deadband, and `store_on_start` used by that run.
