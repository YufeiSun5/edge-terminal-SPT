...
POST /api/v1/main-server/report-jobs/enqueue
  -> 读取 report-readiness 真实同步数据
  -> 为每条 detection_run_report_requests 创建/复用 main_report_jobs
  -> worker 重新检查 readiness
  -> 数据未到位：status=waiting，稍后重查
  -> 全程写 main_report_job_events 生命周期事件
  -> 数据到位：生成 JSON manifest + xlsx，status=success，artifact_ref 指向 xlsx
```

`main_report_jobs` 是主服务器自有表，用于记录 `pending/waiting/running/success/failed`、幂等 `job_key`、重试次数、下一次运行时间、产物路径和错误信息。`main_report_job_events` 是主服务器自有报表任务事件表，用于记录 `enqueued/started/waiting/succeeded/failed/retried`、级别、消息和 JSON payload；`GET /api/v1/main-server/report-jobs/:id/events` 可供页面轮询任务轨迹，也为后续主服务器通知扇出提供来源。当前成功 job 的 `artifact_ref/artifact_name` 指向 `.xlsx`，同目录保留 JSON manifest，`succeeded` 事件 payload 带 `manifest_ref/manifest_name`；后续还需要主服务器自有通知中心发布开始、完成或失败通知，并继续细化客户模板公式的单元格映射规则。
