...
数据库同步模式、表所有权和分阶段改造细节见 [边缘端与主服务器数据库同步及改造计划](./边缘端与主服务器数据库同步及改造计划.md)。主服务器当前能力的收口验收、数据来源、权限、edge 路由、错误码和非压测 smoke 标准见 [主服务器收口验收矩阵](./主服务器收口验收矩阵.md)。
...
- `POST /api/v1/main-server/report-jobs/enqueue`、`GET /api/v1/main-server/report-jobs`、`GET /api/v1/main-server/report-jobs/:id`、`GET /api/v1/main-server/report-jobs/:id/events`、`GET /api/v1/main-server/report-jobs/:id/artifact`、`POST /api/v1/main-server/report-jobs/:id/retry`：主服务器报表 worker 首段接口。入队接口读取上述 readiness 的真实同步数据，为每条 `detection_run_report_requests` 创建幂等 `main_report_jobs`；worker 会等待同步数据到位、失败重试，把 `enqueued/started/waiting/succeeded/failed/retried` 生命周期写入主服务器自有 `main_report_job_events`，并生成 JSON manifest 和可下载 `.xlsx` 产物。若同步的 `sys_report_templates.file_ref` 指向可访问 workbook，worker 会在模板上追加运行数据页；否则生成默认 workbook。artifact 下载只读取成功 job 的主服务器自有 `.xlsx` 产物，且路径必须在 `reports.artifact_dir` 下。报表 `started/succeeded/failed` 事件会生成主服务器自有 `main_report_notifications/main_report_notification_recipients`，用于主服务器用户未读/已读；它不是边缘同步通知表。该任务表、事件表、通知表和产物属于主服务器，不回写边缘端同步业务表；客户模板公式的单元格映射契约仍需后续细化。
- `GET /api/v1/main-server/report-notifications`、`GET /api/v1/main-server/report-notifications/unread-count`、`POST /api/v1/main-server/report-notifications/:id/read`、`POST /api/v1/main-server/report-notifications/read-all`：主服务器报表通知接口，要求主服务器 JWT 和 `view_history` 权限。列表支持 `unread/job_id/level/limit/offset`，已读写入只更新主服务器自有 recipient 表，不修改同步来的 `sys_notification_recipients.read_at`。
...
- `GET /api/v1/gateways`：主服务器已改为网关运行态 service-token 镜像，不读同步库伪造在线状态。它按 `edge_instance_id/project/task` 解析目标边缘，再调用边缘端 `GET /api/v1/edge-control/gateways` 读取 `mqtt.Manager.Status()`，返回当前 active、last_error、订阅主题等真实内存状态。
...
- `GET /api/v1/ws`：主服务器统一 WebSocket facade，要求主服务器 JWT 或浏览器 `access_token` 查询参数，按 `edge_instance_id/project_id/task_id` 路由到对应边缘端 service-token WS。实时订阅桥接边缘端实时消息并补 `edge_instance_id`；`command.*` 写操作会转成受控 edge-control HTTP 命令，复用边缘端幂等、审计和执行链路，不裸代理浏览器 WS 到边缘端。
...
- `GET /api/v1/notifications`、`GET /api/v1/notifications/unread-count`：主服务器侧受保护通知镜像接口，按当前主服务器登录用户的同步 `user_id` 查询 `sys_notifications/sys_notification_recipients`，支持 `unread/type/level/project_id/from/to/keyword/limit/offset`，按 edge context 排除显式归属其他边缘项目的通知，响应保留 `var_id_text` 且 `payload` 为 JSON 对象。已读状态优先读取主服务器自有 `main_notification_reads` 覆盖表，再回退同步接收人表。
- `POST /api/v1/notifications/:id/read`、`POST /api/v1/notifications/read-all`：主服务器写入自有 `main_notification_reads` 覆盖表，支持单条已读和按 `type/level/project_id/from/to/keyword` 分类已读。主服务器仍不直接写同步来的 `sys_notification_recipients.read_at`。报表 worker 继续使用独立的 `main_report_notifications/main_report_notification_recipients` 解决主服务器报表通知的未读/已读；它不替代边缘同步通知镜像。
...
- 更多同步库诊断或只读路由仍按前端主服务器页面需要继续移植；旧迁移期 query proxy 已关闭，未移植接口会明确返回 `501 main_server_query_route_not_implemented`。边缘同步通知的主服务器已读覆盖已由 `main_notification_reads` 实现，后续只需按页面体验继续完善通知聚合，不再把已读能力列为未实现。
- 主服务器客户模板公式单元格映射，以及 station view 模板发布/reload/冲突策略。
...
当前已实现主服务器报表 worker 首段：

```text
POST /api/v1/main-server/report-jobs/enqueue
  -> 读取 report-readiness 可见数据
  -> 为每条 detection_run_report_requests 创建/复用 main_report_jobs
  -> worker 重新检查 readiness
  -> 数据暂不可见：status=waiting_for_sync，稍后重查
  -> 全程写 main_report_job_events 生命周期事件
  -> 数据到位：生成 JSON manifest + xlsx，status=succeeded，artifact_ref 保存 xlsx 的 artifact_key
```

`main_report_jobs` 是主服务器自有表，用于记录 `pending/waiting_for_sync/generating/succeeded/failed`、幂等 `job_key`、重试次数、下一次运行时间、产物 `artifact_key` 和错误信息；查询边界临时兼容旧 `waiting/running/success`。`main_report_job_events` 是主服务器自有报表任务事件表，用于记录 `enqueued/started/waiting/succeeded/failed/retried`、级别、消息和 JSON payload；`GET /api/v1/main-server/report-jobs/:id/events` 可供页面轮询任务轨迹，也为主服务器通知扇出提供来源。当前成功 job 的 `artifact_ref/artifact_name` 指向 `.xlsx` artifact key，`succeeded` 事件 payload 带 manifest 的 `artifact_key/manifest_name`；后续还需要继续细化客户模板公式的单元格映射规则。

## 7. 数据是否可见的判断

主服务器报表 worker 不能只看 `sys_detection_tasks.status=stopped`，还要确认关键数据在主服务器查询视角已经可见。
