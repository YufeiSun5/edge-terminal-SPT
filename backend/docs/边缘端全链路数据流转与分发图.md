...
- `POST /api/v1/main-server/report-plan-imports/parse` 是主服务器报表设置的计划导入解析草稿接口。它只写主服务器本地 `plan-imports/...` artifact，读取同步库项目、变量和模板做匹配。当前支持 `.xlsx`，能解析项目编码、检测编号、出厂编号、变量、上下限、模板编码、报表名和 `param.*`，上下限支持 `10~20%`、`10±5`、`10±5%`、`>=10`、`<=20`、`10 +5/-3`，返回行级 `issues/confidence/needs_confirmation` 供前端人工确认。
