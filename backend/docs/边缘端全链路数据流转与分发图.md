...
首批多向配置表使用节点号段 BIGINT ID：`node_id * id_block_size + sequence`。父子表写入顺序固定为先子表/明细，最后更新父表 `version/updated_at`。运行态表新增 `edge_instance_id` 后，边缘启动只恢复本机 running 检测任务和本机 enabled task flows；主服务器仍可聚合同步库并按 `edge_instance_id` 过滤。实时值、WS、检测控制、KIO/变量写入、`task-flows/runtime` 不走数据库同步写入。
