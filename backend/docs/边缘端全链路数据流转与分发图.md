...
- 主服务器报表通知属于主服务器自己的通知中心，当前由 `main_report_notifications/main_report_notification_recipients` 承载，不写同步来的边缘通知表。报表通知 payload 必须携带 `task_id/request_id/test_no/project_id/project_code/report_name/template_code`，前端在 `main_server` 模式下把 `report.job` 合并到现有顶栏通知和通知中心，点击跳转历史任务的 `Excel 报表` tab。
