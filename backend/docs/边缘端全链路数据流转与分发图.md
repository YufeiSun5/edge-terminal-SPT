...
低频配置链路：
边缘或主服务器配置写接口
  -> 同步配置表 sys_station_view_* / sys_detection_standards/items / sys_task_flows/vars
  -> 数据库同步软件多向分发
  -> 边缘 watcher 观察父表 version/updated_at
  -> 任务流 reload / 工位重新查询 / 检测标准影响未来启动

运行控制链路：
主服务器页面或 WS
  -> 主服务器权限校验和 edge_instance_id 路由
  -> edge-control service-token HTTP/WS
  -> 边缘端 DetectionRunsService / VariableWriteService / TaskFlowExecutor / KIO
  -> 边缘写运行结果、报警、历史、任务 run
  -> 数据库同步软件单向同步到主服务器聚合查询

待检测计划链路：
外部系统或主服务器写入 sys_detection_plans(status=pending)
  -> 数据库同步软件分发到边缘
  -> 边缘工位开始弹窗查询 pending 计划
  -> 边缘本地 POST /api/v1/detection-plans/:id/start
     或主服务器 POST /api/v1/detection-plans/:id/start
        -> edge-control service-token
        -> POST /api/v1/edge-control/detection-plans/:id/start
  -> 计划 pending -> starting
  -> DetectionPlansService 解析 standard_code/report_request_json
  -> VariableWriteService 写 watched STRING 虚拟变量（默认 task_request）
  -> TaskFlowExecutor data_change 倒排索引命中任务流
  -> builtin.write_control_variables（可选 PLC 下设）
  -> builtin.start_detection_run 创建 sys_detection_tasks 并装入 TaskManager
  -> 计划观察到真实任务后 started + started_task_id；失败回 pending + error_message
  -> 数据库同步软件把计划状态和真实任务同步回主服务器

运行暂存链路：
边缘或主服务器工位页检测前预载
  -> GET/PUT/DELETE /api/v1/runtime-drafts/:namespace
  -> 边缘端 RuntimeDraftService 内存 map
  -> 检测启动 payload runtime_draft 引用
  -> DetectionRunsService 或 builtin.start_detection_run 消费并冻结到检测任务
  -> 启动成功后清空对应项目暂存；失败保留
```

首批多向配置表使用节点号段 BIGINT ID：`node_id * id_block_size + sequence`。父子表写入顺序固定为先子表/明细，最后更新父表 `version/updated_at`。运行态表新增 `edge_instance_id` 后，边缘启动只恢复本机 running 检测任务和本机 enabled task flows；主服务器仍可聚合同步库并按 `edge_instance_id` 过滤。`sys_detection_plans` 是低频业务计划交接表，不是运行态抢占锁；真正检测仍以边缘任务流创建的 `sys_detection_tasks` 为准。实时值、WS、检测控制、KIO/变量写入、`task-flows/runtime` 不走数据库同步写入。

运行暂存区同样不走数据库同步，也不由主服务器保存副本。中心页面只通过 service-token 桥接读取/保存目标边缘当前内存状态；目标边缘离线时不接受离线暂存修改。
...
| `Runtime Draft` | 运行暂存。边缘后端内存里的通用页面草稿区，第一期用于工位页检测前预载配置；不写数据库，检测正式开始成功后消费并清空。 |
...
flowchart LR
  ...
  后端HTTP --> 数据库
  后端HTTP --> 运行暂存["RuntimeDraftService\n检测前内存暂存"]
  后端HTTP --> 检测服务["检测服务\n启动、停止、暂停、恢复"]
  后端WS --> 检测服务
  后端WS --> 写入服务["变量写入服务\n校验后写虚拟变量或 KIO"]
...
- 边缘端 WS 快照频率由 `realtime.ws_snapshot_interval_ms` / `EDGE_WS_SNAPSHOT_INTERVAL_MS` 控制，默认 `500ms`，允许 `250..5000ms`。现场 MQTT/KIO 当前按 1 秒级变化时，工位右侧表格和驾驶舱监测表以 WS 为主通道，目标 p95 显示延迟低于 1 秒；曲线仍可在前端按 1000-2000ms 节流，不承诺和 WS 快照同频重绘。
- 前端统一通过 shared realtime client 订阅 WS。shared client 自动重连，退避为 `500ms -> 1s -> 2s -> 5s -> 10s -> 30s` 并带 jitter，浏览器 `online` 事件会立即触发重连；unsubscribe 后停止重连并释放 socket。重连时重新读取当前 access token，命令型 WS `command.*` 仍保持一次性请求，不自动重发，避免重复控制现场设备。
- `GET /api/v1/realtime/variables` 在工位和驾驶舱中只作为首屏和 WS 断线/重连期间的 2 秒兜底。WS 正常后，前端停止高频 HTTP 轮询；WS 帧先进入最新值缓存，UI 最多每 500ms commit 一次，中间帧丢弃，只显示最新值。
- `POST /api/v1/auth/refresh` 用于长期运行会话续期；当前 access token 仍有效时返回新 `access_token/token_type/expires_in/user/permissions`，过期或无效时返回标准 401。主服务器后端保持同名合同，保证共用前端在边缘模式和主服务器模式下都能长时间运行。
...
工位显示配置是低频配置链路，不进入 MQTT 清洗热路径，不直接修改 `TagManager`、检测运行快照、报警状态或存储路由。新 API/DTO 使用 `layout_area=card_pool|list_layout`，不再向前端暴露旧 `region_key` 或左右区域语义；数据库中的旧字段只作为迁移和历史同步兼容。工位页拖拽排序和点击置顶不再使用 `localStorage` 或前端兜底偏好，而是通过 `PUT /api/v1/station-view/items` 更新 `sys_station_view_items.sort_order/pinned`，由数据库配置决定下次打开和切项目后的显示顺序。

### 5.2 工位检测前 Runtime Draft 暂存链路

```text
工位页打开
  -> GET /api/v1/runtime-drafts/station.detection_preload?scope_type=project&scope_id=<project_id>
  -> 边缘 RuntimeDraftService 返回当前内存草稿或 404

点击“应用预载”
  -> PUT /api/v1/runtime-drafts/station.detection_preload
  -> expected_revision 乐观锁
  -> RuntimeDraftService 写入 namespace + project scope

点击“开始检测”
  -> watched STRING 任务请求或 HTTP/edge-control start payload 带 runtime_draft
  -> 后端按 project_id 推导 scope，校验 revision/config_hash/变量归属/变量重复
  -> 转成本次检测冻结 standard_items + process_params
  -> sys_detection_tasks 和冻结快照创建成功
  -> 清空该 project draft
```

该链路只服务检测前预览和开工默认值。检测运行中，前端必须完全切换到 `GET /api/v1/detection-runs/current?project_id=...` 返回的冻结 `standard_items`、`storage_routes` 和实时/历史数据，不能继续读 Runtime Draft。主服务器模式下浏览器仍只访问主服务器同名 API；主服务器校验权限和目标边缘后转发到边缘 `edge-control/runtime-drafts`，不保存中心暂存区，也不为离线边缘排队。正常线路要求 watched STRING payload 显式携带 `runtime_draft` 和本次启动意图的 `end_policy`，任务流 step 显式把这些字段映射给 `builtin.start_detection_run`；执行器的 `task_params.runtime_draft` 和 duration 推断只作为旧配置兼容兜底。Runtime Draft 的 `config_hash` 必须与当前检测标准一致，不一致时后端返回 `409/code=runtime_draft_stale`，前端必须要求重新应用检测配置，不能自动退回到标准项或忽略草稿继续开工。任何兜底使用都会在 `task_flow_runs.script_logs` 和 start step result 中留下 `WARN start_detection fallback ...`，便于测试结束后核对。
...
TagManager 当前稳定值
  ...
  -> 判断该变量是否允许存储
       - detection_run_standard_items.check_enabled：本次检测变量强制存储
       - detection_run_standard_items.store_enabled：本次检测标准显式要求存储
       - detection_run_storage_routes：项目已启用的独立存储变量也随任务冻结
...
存储配置不属于变量属性，属于 `sys_storage_routes`，检测启动时冻结到 `detection_run_storage_routes`。启动时后端会保证每个参与检测的变量有一条默认 route；`check_enabled=true` 的检测变量即使标准项里 `store_enabled=false` 也会强制启用默认存储路由并进入本次任务历史。除此之外，项目里当前已经 `enabled=true` 的 `sys_storage_routes` 也会按本次 `task_id/test_no` 冻结，即使这些变量不在检测标准项里，也作为独立过程变量随本次检测入库。用户自定义 route 只要 enabled，也会被冻结并进入存储热路径。
...
- `check_cycle_ms` 只属于检测标准/运行快照的业务判定节拍，绝对不作为历史入库周期。存储周期只来自 `sys_storage_routes.cycle_ms`；默认 route 建议值为 `20000ms`，冻结任务时若自动启用 disabled default route，也只能使用 route 自己的 `cycle_ms` 或该默认值。
...
flowchart TB
  ...
  兼容入口["兼容/调试入口\nHTTP / WS 检测命令"] --> 配置确认["配置确认\n本地 hash 重算 / 主服务器等待同步"]
  配置确认 --> 检测服务
  主服务器控制["主服务器控制\nedge-control HTTP/WS"] --> 配置确认
  ...
  配置确认 --> 配置等待通知["配置等待/失败通知\nconfig_waiting/config_not_ready"]
  检测服务 --> 配置应用通知["配置应用通知\nconfig_applied/config_apply_failed"]
...
- `STRING` 指令 JSON 支持 `project_id`、`factory_no`、`customer_name`、`device_model`、`test_no`、`standard_id`、`config_enabled`、`config_code`、`config_name`、`config_version`、`config_hash`、`custom_items`、`limit_check_enabled`、`end_policy`、`duration_sec`、`qualified_hold_ms`、`operator_note`、`process_params`、`plc_writes` 等字段。这里的“配置参数”是一次任务请求的业务参数包，不只是检测标准项。
- `factory_no` 是可重复业务查询字段；`test_no` 是检测编号，允许空，由边缘端自动生成唯一值。`customer_name` 和 `device_model` 可空。
- `config_enabled` 在开始检测时确定；`false` 表示本次检测不允许后续应用检测配置。`config_status` 在任务行中表示 `disabled|pending|waiting|applied|failed`。
- 边缘本地启动可以读取本地 `standard_id`，启动前重算本地完整配置 hash 并冻结 revision 1。主服务器发起启动不能只依赖同步库行 ID，必须通过 HTTP/WS 控制命令携带 `config_code + config_version + config_hash`；边缘端用本地标准和标准项重算 hash，一致才启动，不一致进入 `config_waiting` 后台等待，HTTP 返回 `202 accepted` 和 `command_id`。
- `config_waiting` 等待期间不使用旧配置启动；超时后命令失败为 `config_not_ready`。等待时只发首次等待通知和最终结果通知，不按每次重试刷屏。等待时间来自内存运行参数 `detection_config_ready_timeout_ms` 和 `detection_config_ready_interval_ms`，默认 60000/5000。
- 运行中修改检测配置必须走显式 `apply_detection_config` 控制命令并绑定 `task_id` 或明确 active project。无运行任务拒绝；当前任务 `config_enabled=false` 拒绝并返回 `config_disabled_for_run`；hash 已同步则关闭旧 revision、插入新 revision、更新 `TaskManager`；hash 未同步则进入 `config_apply_waiting`，等待期间当前检测继续使用旧已应用 revision。
- 数据库同步只分发配置和历史；检测启动、配置等待和运行中应用配置只通过 HTTP/WS 控制链路驱动。后续系统检测配置表变化不会自动影响已运行任务，必须有显式应用命令。
- `process_params` 是本次检测的低频工艺/报表参数，例如进风口面积、工装编号、环境修正参数；它会随检测冻结进 `custom_config_json`，供报表、计算和审计追溯使用。
- `report_request` 是本次检测要交给主服务器生成报表的请求清单；后端会在检测启动时冻结到 `detection_run_report_requests`，一行表示一份报表请求。正式格式是 `report_request.reports[]`，每项可指定 `template_id` 或 `template_code`、一个或多个变量、以及 `params` 参数对象。报表变量输入只接受 `var_name` 或 `variable_names[]` 这类业务变量名；后端按当前 `project_id + var_name` 解析目标项目自己的 enabled 变量并冻结真实 `var_id`。如果找不到或同一项目存在多个 enabled 同名变量，启动/追加报表请求失败，不按第一个变量兜底。`var_id/var_id_text/var_ids` 只允许出现在冻结后的快照和追溯输出中，不作为模板或报表请求输入合同。
...
- 主服务器报表通知属于主服务器自己的通知中心，当前由 `main_report_notifications/main_report_notification_recipients` 承载，不写同步来的边缘通知表。报表通知 payload 必须携带 `task_id/request_id/test_no/project_id/project_code/report_name/template_code`。报表通知 HTTP 列表、未读数和批量已读支持 `event_type` 过滤，允许重复参数或逗号分隔，例如 `event_type=started&event_type=succeeded`；后端按 `main_report_job_events.event_type` 过滤，不要求前端全量分页后再筛。通知中心列表和未读数还可传 `dedupe=job_event`，按 `job_id + event_type` 只返回最新一条通知，用于隐藏等待/重试期间重复写入的 started 通知；批量已读不依赖去重，仍可清理全部匹配重复未读项。前端在 `main_server` 模式下只把 `started/succeeded` 的 `report.job` 合并到现有顶栏通知和通知中心，点击跳转历史任务的 `Excel 报表` tab。
...
- 检测配置页的数据源是 `detection-standards/projects/variables/report-templates` 这些 HTTP API，不直接查数据库。编辑某个具体项目的检测配置时，落库项会保存该项目当前变量的 `var_id` 作为同步快照；跨项目复用、模板映射和报表请求的业务合同以 `var_name` 为准。开工冻结时后端按目标 `project_id + var_name` 重新解析目标项目自己的 `var_id`，显示名按 `display_name/display_name_en/display_name_ja` 做三语 fallback。
...
- `builtin.start_detection_run` 支持自定义检测项、运行暂存 `runtime_draft`、是否启用上下限、固定时长结束和合格持续时长结束。正式配置应在 step params 中显式映射这些字段；旧任务流步骤配置缺字段时，执行器可从 watched STRING 的 `task_params` 兜底读取并写入 `WARN start_detection fallback ...` 日志，不能把兜底当作长期主路径。
...
- 登录、用户、SSO。
- 项目管理。
- 变量属性管理：`GET /api/v1/variables` 是低频配置列表，不是跨边缘聚合视图。边缘端会按本机 `Auth.EdgeInstanceID` 过滤已分配项目变量；显式传入非本机 `edge_instance_id` 返回空列表，不暴露其他边缘变量。未分配变量池仍允许显示本机 gateway、legacy 未归属 gateway 和 `gateway_id=0` 的虚拟/手工候选变量，供当前边缘继续完成变量分配；`project_id + edge_instance_id` 不匹配时同样不会返回变量。
- 存储路由管理。
...
- 主服务器报表任务基于同步到位的数据生成报表产物。当前已实现 `main_report_jobs` 队列、`main_report_job_events` 生命周期事件、JSON manifest 追溯文件、可下载 `.xlsx` 产物和主服务器自有报表通知；产物通过主服务器本地 Afero `ArtifactStore` 写入 `data/report-assets`，业务表保存 `artifact_key`，不以本机绝对路径作为跨端合同。主服务器 report worker 会在启动 schema 检查时确保系统默认模板 `SPINDLE_DEFAULT_REPORT v1`，文件引用为 `templates/default-report-template.xlsx`；该模板只能作为请求显式选择的系统模板，缺少请求模板或模板文件不可达时 job 失败，不再静默使用默认模板兜底。EB-069 首段已支持 `ReportPackage` 留痕，按 `sys_report_templates.params_schema_json.cell_mapping` 或本次 `params_json.cell_mapping` 指定 Excel sheet/cell 填入任务编号、参数、变量指标和上下限；连续合格两小时窗口已基于同步历史扫描实现。客户模板 cell mapping、默认模板 smoke 和缺模板失败已由主服务器报表单元/路由测试覆盖。
...
- `POST /api/v1/main-server/report-jobs/enqueue`、`GET /api/v1/main-server/report-jobs`、`GET /api/v1/main-server/report-jobs/:id`、`GET /api/v1/main-server/report-jobs/:id/events`、`GET /api/v1/main-server/report-jobs/:id/artifact`、`POST /api/v1/main-server/report-jobs/:id/retry` 和 `POST /api/v1/main-server/report-jobs/:id/regenerate` 是主服务器自有报表 worker 接口。它们只写 `main_report_jobs/main_report_job_events/main_report_notifications/main_report_notification_recipients`，不写 `detection_run_report_requests`、`sys_detection_tasks` 或其他边缘同步业务表；worker 每次处理都会重查 readiness，数据未到位保持 `waiting_for_sync`，并记录 `enqueued/started/waiting/succeeded/failed/retried` 生命周期事件；`started/succeeded/failed` 事件会生成主服务器报表通知，通知 payload 带任务、项目、报表名和模板上下文；到位后通过 `ArtifactStore` 写 JSON manifest 和 `.xlsx`，其中 `artifact_ref` 保存 `.xlsx` 的 `artifact_key`，`succeeded` 事件 payload 带 manifest 的 `artifact_key`。`regenerate` 会创建带 `parent_job_id/generation_type=params_override/params_override_json` 的新 job，用覆盖参数生成新 artifact，不覆盖旧文件、不回写原始报表请求。`RunDueOnce` 会先自动扫描已停止、已有 `detection_run_report_requests` 且尚无 job 的任务并入队。新 job 状态使用 `pending/waiting_for_sync/generating/succeeded/failed`，查询边界临时兼容旧 `waiting/running/success`。EB-069 后 manifest 额外包含 `report_package`，Excel 额外包含隐藏的 `Report_Package` 追溯页，并按模板级 `params_schema_json.cell_mapping` 或请求级/覆盖级 `params_json` cell mapping 填客户模板指定单元格、按 `cell_mapping.chart_sheet/chart_cell` 把曲线图片插入业务 sheet；历史 Excel 报表页只读加载该 `.xlsx` artifact，Luckysheet adapter 会把工作簿图片和 sheet anchor 转成预览图片对象，并只展示 Excel 可见 sheet。artifact 下载只允许读取成功 job 且 `artifact_key` 可由主服务器本地存储解析；未成功返回 `409 report_artifact_not_ready`，文件缺失或越界返回 `404 report_artifact_unavailable`。
- `POST /api/v1/main-server/download-packages` 是历史详情“数据下载”的主服务器打包入口。第一版同步返回 zip，不新增 package 状态表；请求体传 `task_id` 和选择项 `keys`，服务端从主服务器查询视角读取任务快照、检测标准/上下限/存储路由/报表请求、历史数据 CSV、事件/报警记录，并把选中的成功报表 job `.xlsx` artifact 和 job events 放入 zip。zip 内固定包含 `download-manifest.json`；单独曲线 PNG 尚未独立落盘时不生成伪文件，只在 manifest 的 `skipped` 中说明，正式曲线以报表 `.xlsx` 内嵌图片为准。缺失或未成功的报表 artifact 沿用 `report_artifact_not_ready/report_artifact_unavailable` 错误，不生成半真包。
...
- `GET /api/v1/report-templates` 是主服务器侧同名只读报表模板接口，读取当前 `sys_report_templates`，支持 `enabled/keyword` 查询，返回模板编码、版本、文件引用和 `params_schema_json`，供主服务器页面和报表 worker 理解模板参数。`/api/v1/main-server/report-templates*` 是主服务器报表设置的模板资产管理接口：支持上传 xlsx 模板副本、保存 `artifact_key/file_sha256/file_size`、更新 mapping JSON、下载模板副本；当前 `template_code` 仍是唯一行，上传同编码会更新当前模板版本，不可变历史版本表后续再补。
- `POST /api/v1/main-server/report-plan-imports/parse` 是主服务器报表设置的计划导入解析草稿接口。它只写主服务器本地 `plan-imports/...` artifact，读取同步库项目、变量和模板做匹配。当前支持 `.xlsx`，能解析项目编码、检测编号、出厂编号、变量、上下限、模板编码、报表名和 `param.*`，上下限支持 `10~20%`、`10±5`、`10±5%`、`>=10`、`<=20`、`10 +5/-3`，返回行级 `issues/confidence/needs_confirmation` 供前端人工确认。对于真实客户报告书这类无表头、合并单元格较多的 Excel，前端可随 multipart 表单提交 `mapping_json`：`sheet` 指定 sheet，`common` 指定公共字段单元格，`rows[]` 指定每个变量行的 `variable/limit/setting/unit` 单元格和 `params` 单元格。后端按配置取值后进入同一套匹配和确认逻辑。
- `POST /api/v1/main-server/report-plan-imports/confirm` 接收前端确认后的导入行，按项目和检测编号分组创建 `sys_detection_standards/sys_detection_standard_items` 检测配置，写入走主服务器同步 ID 和 `sync_scope/edge_instance_id/updated_by_*` 元数据。该接口复用 `StationViewQuery.CreateDetectionStandard`：先校验 `project_id/project_code/edge_instance_id`、项目 enabled、变量存在且 enabled 且属于目标项目，再写父表非水位字段和子表，按边缘端同一稳定 payload 算法计算 `config_hash`，最后更新父表 `version/config_hash/updated_at` 作为同步完成水位。检测配置创建后继续创建 `sys_detection_plans(status=pending)`，写入生成的 `standard_code`，并把导入文件里的模板、报表名、变量和 `param.*` 组合为 `report_request_json`。边缘从计划开工时先解析本地检测配置，再把 `report_request_json` 冻结到真实任务的 `detection_run_report_requests`；仍不得把 `sys_detection_tasks` 的执行态写成伪 `pending_test`。
...
- `GET /api/v1/history/data` 是主服务器侧同名只读历史接口，读取同步库真实历史；策略与边缘端一致：先根据 `detection_run_storage_routes` 读项目动态宽表并还原为 `HistoryData`，宽表无数据时回退 `rt_history_data`。支持按可重复业务 `factory_no` 查询同一出厂编号的多次检测数据。响应包含 `items/count/limit` 和 `var_id_text`。
...
- `GET /api/v1/detection-runs`、`GET /api/v1/detection-runs/:id`、`/summary`、`/features`、`/events`、`/storage-routes`、`/report-requests`、`/notes` 是主服务器侧同名只读检测任务接口族，读取同步库真实 `sys_detection_tasks/detection_run_*` 表；列表支持 `factory_no` 并按 edge context 过滤显式归属其他边缘的项目，详情附带标准项、存储路由、备忘录摘要、报表结果和报表请求快照；notes 只读同步 `detection_run_notes`，`POST /api/v1/detection-runs/:id/notes` 返回 `501 main_server_detection_note_write_unsupported`。
...
- `POST /api/v1/edge-control/detection/start|apply-config|stop|abnormal-stop|pause|resume|mute-alarms|update-limits|refresh-features|report-requests` 和 `POST /api/v1/edge-control/variables/write` 是主服务器侧受控现场命令入口。它们不写主服务器镜像库，只调用边缘端同名 `/api/v1/edge-control/*`；检测启动和应用配置可能因配置 hash 未同步返回 `202 accepted`，后续命令状态通过 `edge_control_commands` 查询；缺 service token 返回 `503 edge_control_token_missing`，边缘端不可达返回 `502 edge_backend_unavailable`。
- `POST /api/v1/detection-runs`、`POST /api/v1/detection-runs/:id/apply-config`、`POST /api/v1/detection-runs/:id/stop`、`POST /api/v1/detection-runs/:id/abnormal-stop`、`POST /api/v1/detection-runs/:id/pause`、`POST /api/v1/detection-runs/:id/resume` 是主服务器为共用前端提供的同名检测控制入口。主服务器只校验用户 JWT 和权限、补操作者字段和 `command_id`、把 URL `:id` 转成 `payload.task_id`，实际检测启动、应用配置、停止、暂停、恢复仍进入边缘端 `/api/v1/edge-control/detection/*`。
...
当前边缘端已新增 `detection_run_report_requests`，中文含义是“检测运行报表请求快照”。它可在检测启动时生成，也可由主服务器后续通过 `/api/v1/edge-control/detection/report-requests` 为已有检测任务追加，一行表示某次检测需要主服务器生成一份报表；这份报表可以绑定一个变量，也可以绑定多个变量。记录会冻结模板身份、`variables_json` 变量组和 `params_json` 公式参数，`ext_1/ext_2/ext_3` 仅作为旧兼容预留字段。主服务器看到它同步到镜像库后，可调用 `POST /api/v1/main-server/report-jobs/enqueue` 创建或复用 `main_report_jobs`，worker 也会在轮询时自动发现已停止且尚无 job 的报表请求；worker 再用 `GET /api/v1/main-server/report-readiness?task_id=...` 等价检查历史数据、报警、摘要、特征值是否到位。处理过程写入 `main_report_job_events`，页面可通过 `GET /api/v1/main-server/report-jobs/:id/events` 查询事件轨迹；确认后生成 JSON manifest 和可下载 `.xlsx`。报表开始、成功、失败会写入 `main_report_notifications/main_report_notification_recipients`，页面可通过 `main-server/report-notifications` 查询和标记已读；通知中心默认只用 `event_type=started,succeeded` 查询开始和完成记录，失败事件仍保留在主服务器报表事件/通知表中供专门页面追溯。EB-069 首段后 `.xlsx` 会写入运行数据页和 `Report_Package` 完整追溯页，并可按模板级 `params_schema_json.cell_mapping` 或请求级 `params_json.cell_mapping` 填客户模板指定单元格、按 `chart_sheet/chart_cell` 插入曲线图片；映射 schema 见 [报表业务口径与客户模板单元格映射设计](./报表业务口径与客户模板单元格映射设计.md)。
