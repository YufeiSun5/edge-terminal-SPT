...
5. 通知项目成员 UI 收敛：后端已补 `sys_project_members` 和 `GET/PUT /api/v1/projects/{id}/members`，`target_type=project` 在配置成员后会按 `notify_enabled` 精确分发；前端还需要提供项目成员/通知接收配置入口。未配置成员的项目仍临时全员兜底，避免升级后检测通知无人接收。
