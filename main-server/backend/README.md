...
`/api/v1/main-server/report-jobs*` are main-server-owned report worker routes. `POST /enqueue` reads the same real synchronized readiness data and creates idempotent `main_report_jobs` rows, one row per synchronized `detection_run_report_requests` item. The background worker rechecks readiness, waits when mirror data is incomplete, retries transient failures, writes lifecycle rows to `main_report_job_events`, writes a JSON manifest under `reports.artifact_dir`, and generates a downloadable `.xlsx` artifact. When a synchronized `sys_report_templates.file_ref` points to a reachable workbook, the worker appends data sheets to that workbook. If the requested template is missing or unreachable, the worker ensures and uses the system default template `SPINDLE_DEFAULT_REPORT` version `1`, file ref `templates/default-report-template.xlsx`; `generated_default_missing_template` is only a defensive fallback if that default template itself cannot be created or opened. The default template is seeded during `EnsureSchema`, stores its `params_schema_json` cell mapping metadata in `sys_report_templates`, and the file is created under `reports.artifact_dir`. `GET /:id/events` returns the job event timeline. `GET /:id/artifact` downloads the successful job's `.xlsx` artifact and rejects not-ready jobs or paths outside `reports.artifact_dir`. Started, succeeded, and failed report events also create rows in main-server-owned `main_report_notifications/main_report_notification_recipients`; these are not synchronized edge notifications. This worker stage never writes synchronized edge business tables; customer-specific template cell mapping is driven by `detection_run_report_requests.params_json`.
